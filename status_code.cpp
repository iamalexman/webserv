...
/*** curl
 curl -H "name: lorem" -H "Content-Type: text/html" -H "Transfer-Encoding: chunked" -d @_.txt localhost:8080/upload
 * ***/

//curl --resolve localhost:80:127.0.0.1 localhost
//curl -X POST -H "Content-Type: plain/text" --data "BODY IS HERE" localhost:8080/upload
